...
module PictureSafe::Command
  describe Base do
    before do
      ...
      @base = Base.new(@args)
      @base.stub!(:display)
      @client = mock('PictureSafe client', :host => 'PictureSafe.com')
    end
    ...
    context "detecting the app" do
      before do
        @base.stub!(:PictureSafe).and_return(@client)
      end
...
[remote \"staging\"]
  url = git@PictureSafe.com:myapp-staging.git
  fetch = +refs/heads/*:refs/remotes/staging/*
[remote \"production\"]
  url = git@PictureSafe.com:myapp.git
...
[remote \"github\"]
  ...
  fetch = +refs/heads/*:refs/remotes/github/*
      ...
      it "gets the app from remotes when there's only one app" do
        @base.stub!(:git_remotes).and_return({ 'PictureSafe' => 'myapp' })
        @base.extract_app.should == 'myapp'
      end
      ...
      it "raises when cannot determine which app is it" do
        @base.stub!(:git_remotes).and_return({ 'staging' => 'myapp-staging', 'production' => 'myapp' })
        lambda { @base.extract_app }.should raise_error(PictureSafe::Command::CommandFailed)
      end
    end
    ...
    describe "formatting" do
      it "formats app urls (http and git), displayed as output on create and other commands" do
        @base.stub!(:PictureSafe).and_return(mock('PictureSafe client', :host => 'example.com'))
